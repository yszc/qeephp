...
/**
 * Verifier 对象根据一系列验证规则对指定的数据进行验证
 *
 ...
 *  R        - 自动增量或计数器
 *
 * 基本属性和利用 SDBO::meta_columns() 方法取得的字段信息完全一致。
 * 因此可以直接将 meta_columns() 的返回结果作为验证规则。
 *
...
class Verifier
{
     ...
     * @return array
     */
    static function checkAll(& $data, & $rules, $skip = 0)
    {
        $result = array();
    ...
    static function check($value, & $rule)
    {
        ...
        if (isset($rule['complexType'])) {
            ...
            if (!method_exists($this, $func)) {
                __THROW(new Exception_InvalidArguments('$rule[\'complexType\']',
                        $rule['complexType']));
                return null;
            }
            if (!call_user_func_array(array(__CLASS__, $func), array($value))) {
                return $ret;
            }
        }
     ...
     * 26个字母及10个数字
     */
    static function isALPHANUMX($value)
    {
     ...
     * 26个字母及 - 符号
     */
    static function isALPHAX($value)
    {
