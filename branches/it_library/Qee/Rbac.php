...
/**
 * Qee_Rbac 提供基于角色的权限检查服务
 *
 * Qee_Rbac 并不提供用户管理和角色管理服务，
 * 这些服务由 Qee_Rbac_UsersManager 和 Qee_Rbac_RolesManager 提供。
 *
 * @package Core
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
...
class Qee_Rbac
{
    ...
    {
        $this->_sessionKey = Qee::getAppInf('RBACSessionKey');
        $this->_rolesKey = Qee::getAppInf('RBACRolesKey');
    }
    ...
    public function check(& $roles, & $ACT)
    {
        ...
        if ($ACT['allow'] == RBAC_EVERYONE) {
            ...
            if ($ACT['deny'] == RBAC_EVERYONE) {
                require_once 'Qee/Rbac/Exception/InvalidACT.php';
                throw new Qee_Rbac_Exception_InvalidACT($ACT);
            }
