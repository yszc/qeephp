...
class Qee_Log
{
    ...
    {
        $dir = Qee::getAppInf('logFileDir');
        if ($dir == null || $dir == '') {
            // 如果没有指定日志存放目录，则保存到内部缓存目录中
            $dir = Qee::getAppInf('internalCacheDir');
        }
        $dir = realpath($dir);
        if (!is_dir($dir) || !is_writable($dir)) {
            ...
            $this->_logFilename = $this->_logFileDir . DS . Qee::getAppInf('logFilename');
            $errorLevel = explode(',', strtolower(Qee::getAppInf('logErrorLevel')));
            ...
            $maxsize = (int)Qee::getAppInf('logFileMaxSize');
            if ($maxsize >= 512) {
