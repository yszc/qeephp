...
/**
 * Qee_Db_TableLink 封装数据表之间的关联关系
 *
 * Qee_Db_TableLink 是一个完全供 QeePHP 内部使用的类，
 * 开发者不应该直接构造 Qee_Db_TableLink 对象。
 *
 * @package Core
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.2
 */
abstract class Qee_Db_TableLink
{
    /**
     * 该连接的名字，用于检索指定的连接
     *
     ...
     * 构造函数
     *
     * 开发者不应该自行构造 Qee_Db_TableLink 实例。而是应该通过
     * Qee_Db_TableLink::createLink() 静态方法来构造实例。
     *
     * @param array $define
     * @param enum $type
     * @param Qee_Db_TableDataGateway $mainTDG
     *
    ...
    protected function __construct($define, $type, $mainTDG)
    {
        ...
        foreach ($this->_req as $key) {
            if (!isset($define[$key]) || $define[$key] == '') {
                require_once 'Qee/Db/Exception/MissingLinkOption.php';
                throw new Qee_Db_Exception_MissingLinkOption($key);
            } else {
                $this->{$key} = $define[$key];
            }
        }
        ...
        if (is_null($defaultDsnId)) {
            $defaultDSN = Qee::getAppInf('dbDSN');
            if ($defaultDSN) {
                $defaultDSN = Qee::parseDSN($defaultDSN);
    ...
    static function createLink($define, $type, $mainTDG)
    {
        static $typeMap = array(
            HAS_ONE         => 'Qee_Db_HasOneLink',
            BELONGS_TO      => 'Qee_Db_BelongsToLink',
            HAS_MANY        => 'Qee_Db_HasManyLink',
            MANY_TO_MANY    => 'Qee_Db_ManyToManyLink',
        );

        // 检查 $type 参数
        if (!isset($typeMap[$type])) {
            require_once 'Qee/Db/Exception/InvalidLinkType.php';
            throw new Qee_Db_Exception_InvalidLinkType($type);
        }

        // tableClass 属性是必须提供的
        if (!isset($define['tableClass'])) {
            require_once 'Qee/Db/Exception/MissingLinkOption.php';
            throw new Qee_Db_Exception_MissingLinkOption('tableClass');
        }
        ...
        if ($type == MANY_TO_MANY) {
            if (!isset($define['joinTable']) && !isset($define['joinTableClass'])) {
                require_once 'Qee/Db/Exception/MissingLinkOption.php';
                throw new Qee_Db_Exception_MissingLinkOption('joinTable');
            }
        }
    ...
    {
        throw new Qee_Exception(Qee_Exception::t('%s::saveAssocData() not implemented.', get_class($this)));
    }
    ...
    {
        if ($this->init) { return; }
        if (Qee::isRegistered($this->assocTDGObjectId)) {
            $this->assocTDG = Qee::registry($this->assocTDGObjectId);
        } else {
            if ($this->assocTDGObjectId) {
                Qee::loadClass($this->tableClass);
                $this->assocTDG = new $this->tableClass(array('dbo' => & $this->dbo));
                Qee::register($this->assocTDG, $this->assocTDGObjectId);
            } else {
                $this->assocTDG = Qee::getSingleton($this->tableClass);
            }
        }
        $this->init = true;
    }
    ...
    protected function _getFindSQLBase($sql, $in)
    {
        ...
        if ($this->conditions) {
            if (is_array($this->conditions)) {
                $conditions = Qee_Db_SqlHelper::parseConditions($this->conditions, $this->assocTDG);
...
/**
 * Qee_Db_HasOneLink 封装 has one 关系
 *
 * @package Core
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
class Qee_Db_HasOneLink extends Qee_Db_TableLink
{
...
/**
 * Qee_Db_BelongsToLink 封装 belongs to 关系
 *
 * @package Core
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
class Qee_Db_BelongsToLink extends Qee_Db_TableLink
{
    public $oneToOne = true;
...
/**
 * Qee_Db_HasManyLink 封装 has many 关系
 *
 * @package Core
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
class Qee_Db_HasManyLink extends Qee_Db_HasOneLink
{
...
/**
 * Qee_Db_ManyToManyLink 封装 many to many 关系
 *
 * @package Core
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
class Qee_Db_ManyToManyLink extends Qee_Db_TableLink
{
    /**
     * 组合关联数据时是否是一对一
     *
...
EOT;
    ...
    public function init()
    {
        parent::init();
        if ($this->joinTableClass) {
            $this->joinTDG = Qee::getSingleton($this->joinTableClass);
