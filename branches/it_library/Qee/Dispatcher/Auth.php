...
// {{{ includes
require_once 'Qee/Dispatcher/Simple.php';
// }}}

/**
 * Qee_Dispatcher_Auth 分析 HTTP 请求，并转发到合适的 Controller 对象处理
 *
 * @package Core
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
class Qee_Dispatcher_Auth extends Qee_Dispatcher_Simple
{
    /**
     * 用于提供验证服务的对象实例
     *
    ...
    public function __construct(& $request)
    {
        ...
        $this->_auth = Qee::getSingleton(Qee::getAppInf('dispatcherAuthProvider'));
    }
    ...
    public function dispatching()
    {
        ...
        if ($this->check($controllerName, $actionName, $controllerClass)) {
            ...
            // 检查失败
            $callback = Qee::getAppInf('dispatcherAuthFailedCallback');
            ...
            if ($callback) {
                ...
                require_once 'Qee/Dispatcher/Exception/CheckFailed.php';
                throw new Qee_Dispatcher_Exception_CheckFailed($controllerName, $actionName, $rawACT, $roles);
            }
        }
    }
    ...
    {
        $actFilename = Qee::getFilePath($controllerClass . '.act.php');
        if (!$actFilename) {
            if (Qee::getAppInf('autoQueryDefaultACTFile')) {
                $ACT = $this->getControllerACTFromDefaultFile($controllerName);
                if ($ACT) { return $ACT; }
            }

            if (Qee::getAppInf('controllerACTLoadWarning')) {
                trigger_error(sprintf(_ET(0x0701006), $controllerName), E_USER_WARNING);
            }
            return Qee::getAppInf('defaultControllerACT');
        }
    ...
    {
        $actFilename = realpath(Qee::getAppInf('defaultControllerACTFile'));
        if (!$actFilename) {
            if (Qee::getAppInf('controllerACTLoadWarning')) {
                trigger_error(Qee_Exception::t('ACT file \'%s\' is not found.', $controllerName), E_USER_WARNING);
            }
            return Qee::getAppInf('defaultControllerACT');
        }
        ...
        return isset($ACT[$controllerName]) ? $ACT[$controllerName] : Qee::getAppInf('defaultControllerACT');
    }
    ...
    protected function _loadACTFile($actFilename)
    {
        ...
        require_once 'Qee/Rbac/Exception/InvalidACTFile.php';
        throw new Qee_Rbac_Exception_InvalidACTFile($actFilename, $ACT);
    }
}
