...
/**
 * Qee_Dispatcher_Simple 分析 HTTP 请求，并转发到合适的 Controller 对象处理
 *
 * @package Core
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
class Qee_Dispatcher_Simple
{
    /**
     * 保存了请求信息的数组
     *
    ...
    public function __construct(& $request)
    {
        ...
        $controllerAccessor = strtolower(Qee::getAppInf('controllerAccessor'));
        $actionAccessor = strtolower(Qee::getAppInf('actionAccessor'));
    ...
    protected function _executeAction($controllerName, $actionName, $controllerClass)
    {
        ...
        $actionMethod = $actionPrefix . $actionName . Qee::getAppInf('actionMethodSuffix');

        $controller = null;
        do {
            // 载入控制对应的类定义
            try {
                Qee::loadClass($controllerClass);
            } catch (Exception $ex) {
                break;
            }

            // 构造控制器对象
            Qee::setAppInf('Qee.internal.currentControllerName', $controllerName);
            Qee::setAppInf('Qee.internal.currentActionName', $actionName);
        ...
        $callback = Qee::getAppInf('dispatcherFailedCallback');
        if ($callback) {
            // 检查是否调用应用程序设置的错误处理程序
        ...
        if ($controller == null) {
            require_once 'Qee/Exception/MissingController.php';
            throw new Qee_Exception_MissingController($controllerName, $actionName, $this->_requestBackup, $controllerClass, $actionMethod);
        }

        require_once 'Qee/Exception/MissingAction.php';
        throw new Qee_Exception_MissingAction($controllerName, $actionName, $this->_requestBackup, $controllerClass, $actionMethod);
    }
    ...
    public function getControllerName()
    {
        ...
        if ($controllerName == '') {
            $controllerName = Qee::getAppInf('defaultController');
        }
        if (Qee::getAppInf('urlLowerChar')) {
    ...
    public function getActionName()
    {
        ...
        if ($actionName == '') {
            $actionName = Qee::getAppInf('defaultAction');
        }
        return $actionName;
    }
    ...
    {
        $controllerClass = Qee::getAppInf('controllerClassPrefix');
        if (Qee::getAppInf('urlLowerChar')) {
