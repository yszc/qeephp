...
/**
 * Qee_Helper_Image 类封装了针对图像的操作
 *
 * 开发者不能直接构造该类的实例，而是应该用 Qee_Helper_Image::createFromFile()
 * 静态方法创建一个 Qee_Helper_Image 类的实例。
 *
...
class Qee_Helper_Image
{
     ...
     * $image =& Qee_Helper_Image::createFromFile($_FILES['postfile']['tmp_name'], $ext);
     * </code>
     *
    ...
    static function createFromFile($filename, $fileext = null)
    {
        ...
        if (!isset($ext2functions[$fileext])) {
            throw new Qee_Exception(Qee_Exception::t('Invalid image type: %s', $fileext));
        }

        $handle = $ext2functions[$fileext]($filename);
        return new Qee_Helper_Image($handle);
    }
     ...
     * @return array
     */
    public function extractColor($color, $defualt = 'ffffff')
    {
