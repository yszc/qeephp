...
/**
 * 定义 Qee_Helper_FileUploader 和 Qee_Helper_UploadFile 对象
 *
...
/**
 * Qee_Helper_FileUploader 实现了一个简单的、可扩展的文件上传助手
 *
 * 使用方法：
 *
 * <code>
 ...
 *
 * Qee::loadClass('Qee_Helper_FileUploader');
 * $uploader = new Qee_Helper_FileUploader();
...
class Qee_Helper_FileUploader
{
        ...
        if (is_array($_FILES)) {
            foreach ($_FILES as $field => $struct) {
                if (isset($struct['error']) && $struct['error'] != UPLOAD_ERR_NO_FILE) {
                    $this->_files[$field] = new Qee_Helper_UploadFile($struct, $field);
                }
            }
        }
     ...
     * @param string $name
     *
     * @return Qee_Helper_UploadFile
     */
    public function getFile($name)
    {
        if (!isset($this->_files[$name])) {
            require_once 'Qee/Exception/ExpectedFile.php';
            throw new Qee_Exception_ExpectedFile('$_FILES[' . $name . ']');
        }
    ...
    public function batchMove($destDir)
    {
        foreach ($this->_files as $file) {
            /* @var $file Qee_Helper_UploadFile */
