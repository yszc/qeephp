...
/**
 * 定义 Qee_Db_Driver 类和 Qee_Db_Driver_Handle 类
 *
...
/**
 * Qee_Db_Driver 是所有数据库驱动的基础类，并且提供创建特定类型数据库驱动对象的工厂方法
 *
 * @package Database
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.2
 */
abstract class Qee_Db_Driver
{
    /**
     * 可用的查询参数占位符形式
     *
     ...
     *
     * @return Qee_Db_Driver
     */
    static public function getDBO($dsn = null)
    {
        static $qee = false;
        if (!$qee) { $qee = class_exists('Qee'); }

        if (empty($dsn) && $qee) {
            if (Qee::isRegister('QeePHP.DBO_default')) {
                return Qee::registry('QeePHP.DBO_default');
            }
            $dsn = Qee::getAppInf('dbDSN');
            $default = true;
        } else {
            $default = false;
        }
        ...
        if ($dsn == false) {
            require_once 'Qee/Db/Exception/InvalidDSN.php';
            throw new Qee_Db_Exception_InvalidDSN($dsn);
        }

        $dsnid = 'QeePHP.DBO_#' . $dsn['id'];
        if ($qee && Qee::isRegistered($dsnid)) {
            return Qee::registry($dsnid);
        }

        $driver = ucfirst(strtolower($dsn['driver']));
        $class = 'Qee_Db_Driver_' . $driver;
        require_once "Qee/Db/Driver/{$driver}.php";
        $dbo = new $class($dsn);

        if ($qee) {
            Qee::register($dbo, $dsnid);
            if ($default) {
                Qee::register($dbo, 'QeePHP.DBO_default');
            }
        }
        return $dbo;
    }
     ...
     * 则 execute() 执行成功后会返回一个 Qee_Db_Driver_Handle 对象，失败时将抛出异常。
     *
     * Qee_Db_Driver_Handle 对象封装了查询结果句柄，而不是结果集。
     * 因此要获得查询的数据，需要调用 Qee_Db_Driver_Handle 的 fetchAll() 等方法。
     *
     ...
     *
     * @return Qee_Db_Driver_Handle
     */
    abstract public function execute($sql, $inputarr = null);

    /*
     * 进行限定范围的查询，并且返回 Qee_Db_Driver_Handle 对象，出错时抛出异常
     *
     ...
     *
     * selectLimit() 并不直接返回结果集，而是返回 Qee_Db_Driver_Handle 对象。
     * 因此需要调用 Qee_Db_Driver_Handle 对象的 fetchAll() 等方法来获得数据。
     *
     * example:
     * <code>
    ...
    /**
     * 开始一个事务，并且返回一个 Qee_Db_Transaction 对象
     *
     * 使用 beginTrans() 的好处是在 Qee_Db_Transaction 对象销毁时，会自动提交事务或回滚事务。
     *
     * 传统的事务处理代码：
     * <code>
     * function doSometing()
     ...
     *
     * <strong>
     * 使用 beginTrans() 最重要的一点就是必须用一个变量存储 beginTrans() 返回的 Qee_Db_Transaction 对象。
     * </strong>
     *
     * Qee_Db_Transaction 对象也提供了明确的事务提交和回滚操作。具体信息请参考 Qee_Db_Transaction 类的描述。
     *
     * @return Qee_Db_Transaction
     */
    public function beginTrans()
    {
        require_once 'Qee/Db/Transaction.php';
        return new Qee_Db_Transaction($this);
    }
...
/**
 * Qee_Db_Driver_Handle 是封装查询句柄的基础类
 *
 * @package Database
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
abstract class Qee_Db_Driver_Handle
{
    /**
     * 返回记录的形式
     * @var const
     */
    public $fetchMode = Qee_Db_Driver::FETCH_MODE_ASSOC;
    ...
    {
        $this->pushFetchMode();
        $this->fetchMode = Qee_Db_Driver::FETCH_MODE_ARRAY;
        $cols = array();
        while ($row = $this->fetchRow()) {
            $cols[] = $row[$col];
        }
     ...
     * 将两个数据集按照指定字段的值进行组装
     *
     * 该方法主要用于 Qee_Db_TableDataGateway 组装来自两个数据表的数据。
     *
     * @param Qee_Db_Driver_Handle $handle
     * @param array $assocRowset
     * @param string $mappingName
     * @param boolean $oneToOne
     * @param string $refKeyName
     */
    public function assemble(Qee_Db_Driver_Handle $handle, array & $assocRowset, $mappingName, $oneToOne, $refKeyName)
    {
        if ($oneToOne) {
            // 一对一组装数据
