...
/**
 * 定义 Qee_Db_Driver_Mysqli 类、Qee_Db_Driver_Mysqli_Handle 类和 Qee_Db_Driver_Mysqli_Statement 类
 *
...
// {{{ includes
require_once 'Qee/Db/Driver.php';
// }}}

/**
 * Qee_Db_Driver_Mysqli 是 MySQLi 扩展的驱动程序
 *
 * @package Database
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
class Qee_Db_Driver_Mysqli extends Qee_Db_Driver
{
    /**
     * 指示查询参数的样式
     *
    ...
    public function connect()
    {
        ...
        if (!function_exists('mysqli_init')) {
            require_once 'Qee/Db/Exception/ExtNotLoaded.php';
            throw new Qee_Db_Exception_ExtNotLoaded('MySQL Improved', 'mysqli');
        }
        $this->_conn = mysqli_init();
        if (!$this->_conn) {
            require_once 'Qee/Db/Exception/ConnectionFailed.php';
            throw new Qee_Db_Exception_ConnectionFailed('MySQL Improved', 'mysqli');
        }
    ...
    public function selectDB($database)
    {
        if (!mysqli_select_db($this->_conn, $database)) {
            require_once 'Qee/Db/Exception/UseDatabaseFailed.php';
            throw new Qee_Db_Exception_UseDatabaseFailed('MySQL Improved', 'mysqli', $database);
        }
    }
    ...
    public function execute($sql, $inputarr = null)
    {
        if (!empty($inputarr)) {
            ...
            if (!$stmt) {
                $this->_hasFailedQuery = true;
                require_once 'Qee/Db/Exception/Query.php';
                throw new Qee_Db_Exception_Query($sql, mysqli_error($this->_conn));
            }
            $types = '';
            ...
            if (!mysqli_stmt_execute($stmt)) {
                $this->_hasFailedQuery = true;
                require_once 'Qee/Db/Exception/Query.php';
                throw new Qee_Db_Exception_Query($sql, mysqli_stmt_error($stmt));
            }

            return new Qee_Db_Driver_Mysqli_Statement($stmt, $this->fetchMode);
        } else {
            ...
            return new Qee_Db_Driver_Mysqli_Handle($result, $this->fetchMode);
        }
    }
...
/**
 * Qee_Db_Driver_Mysqli_Handle 封装了一个查询句柄，便于释放资源
 *
 * @package Database
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
class Qee_Db_Driver_Mysqli_Handle extends Qee_Db_Driver_Handle
{
    ...
    {
        if ($this->fetchMode == Qee_Db_Driver::FETCH_MODE_ASSOC) {
...
/**
 * Qee_Db_Driver_Mysqli_Statement 封装了一个查询过程对象，便于释放资源
 *
 * @package Database
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.0
 */
class Qee_Db_Driver_Mysqli_Statement extends Qee_Db_Driver_Handle
{
    /**
     * 查询过程可能返回的结果集包含的字段
     *
