...
{
    $obj = Qee::getSingleton('Qee_Language');
    /* @var $obj Qee_Language */
...
function load_language($dictname, $language = '', $noException = false)
{
    $obj = Qee::getSingleton('Qee_Language');
    /* @var $obj Qee_Language */
    return $obj->load($dictname, $language, $noException);
}

/**
 * Qee_Language 提供了语言转换功能
 *
 * @package Core
 * @author 起源科技(www.qeeyuan.com)
 * @version 1.1
...
class Qee_Language
{
    ...
    {
        $autoload = Qee::getAppInf('autoLoadLanguage');
     ...
     * 'languageFilesDir' 指定的目录中。默认的保存目录为 Qee/Languages。
     *
    ...
    public function load($dictname, $language = '', $noException = false)
    {
        ...
        foreach ($dictnames as $dictname) {
            ...
            $filename = Qee::getAppInf('languageFilesDir') . DS . $language . DS . $dictname . '.php';
            if (isset($this->_loadedFiles[$filename])) { continue; }

            if (is_readable($filename)) {
                $dict = include $filename;
                ...
                throw new Qee_Exception_ExpectedFile($filename);
            }
        }
    }
