...
/**
 * Qee_Helper_ImgCode 类实现了一个简单的图像验证码生成器，并带有检查验证方法
 *
 ...
 *     $imgcode =& Qee::getSingleton('Qee_Helper_ImgCode');
 *     $imgcode->image();
 * }
 * </code>
 *
...
class Qee_Helper_ImgCode
{
    ...
    public function __construct()
    {
        if (!function_exists('imagecreate')) {
            throw new Qee_Exception(Qee_Exception::t('GD2 extension not found.'));
        }

        @session_start();

        $this->_code = isset($_SESSION['IMGCODE']) ? $_SESSION['IMGCODE'] : '';
        $this->_expired = isset($_SESSION['IMGCODE_EXPIRED']) ? $_SESSION['IMGCODE_EXPIRED'] : 0;
    }
