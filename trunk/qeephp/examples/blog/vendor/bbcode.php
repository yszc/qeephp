 ...
 *
 *   $Id: bbcode.php 949 2007-10-03 08:49:18Z dualface $
 *
 ***************************************************************************/
...
{
	$tpl = file_get_contents(dirname(__FILE__) . '/bbcode.tpl');
...
function bbencode_first_pass($text, $uid)
{
	...
	$text = preg_replace("#\[color=(\#[0-9A-F]{3,6}|[a-z\-]+)\](.*?)\[/color\]#si", "[color=\\1:$uid]\\2[/color:$uid]", $text);
...
function replace_listitems($text, $uid)
{
    ...
    foreach ($arr as $offset => $line) {
        if (strpos($line, '[*]') !== false) {
            $line = str_replace('[*]', "[*:$uid]", $line) . "[/*:{$uid}]";
        }
        $arr[$offset] = $line;
    }
    return implode("\n", $arr);
    $text = preg_replace('/\[\*\](.+)/', "[*:{$uid}]$1[/*:{$uid}]", $text);
	// $text =
