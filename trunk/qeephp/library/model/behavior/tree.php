...
class Model_Behavior_Tree extends QDB_ActiveRecord_Behavior_Abstract
{
    ...
    function bind()
    {
        ...
        $this->_addDynamicMethod('getPathNodes',      array($this, 'getPathNodes'));
        $this->_addDynamicMethod('getAllChildNodes',  array($this, 'getAllChildNodes'));
        $this->_addDynamicMethod('getSiblingNodes',   array($this, 'getSiblingNodes'));
        $this->_addDynamicMethod('getAllChildsCount', array($this, 'getAllChildsCount'));
        $this->_addDynamicMethod('destroyAllChildNodes', array($this, 'destroyAllChildNodes'));
        $this->_addDynamicMethod('deleteAllChildNodes',  array($this, 'deleteAllChildNodes'));
    }
    ...
    function _before_create(QDB_ActiveRecord_Abstract $obj)
    {
        ...
        $pid_pn = $this->settings['parent_id_prop'];
    ...
    function getParentNode(QDB_ActiveRecord_Abstract $obj)
    {
        ...
        return $this->_meta->find(array(reset($this->_meta->idname) => $obj->{$pid_pn}))->query();
    }
    ...
    function getPathNodes(QDB_ActiveRecord_Abstract $obj)
    {
        ...
        return $this->_meta->find("[{$lft_pn}] < ? AND [{$rgt_pn}] > ?", $obj->{$lft_pn}, $obj->{$rgt_pn})
                           ->all()
                           ->order("[{$lft_pn}] ASC")
                           ->query();
    }
    ...
    function getAllChildNodes(QDB_ActiveRecord_Abstract $obj)
    {
        ...
        return $this->_meta->find("[{$lft_pn}] BETWEEN ? AND ?", $obj->{$lft_pn}, $obj->{$rgt_pn})
                           ->all()
                           ->order("[{$lft_pn}] ASC")
                           ->query();
    }
