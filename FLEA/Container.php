...
/**
 * FLEA_Container 类提供了类的载入、对象注册等服务
 *
 * @package Core
 * @author 廖宇雷 dualface@gmail.com
 * @version 1.0
 */
class FLEA_Container
{
    /**
     * 对象注册表
     *
     ...
     * example:
     * <code>
     * FLEA_Container::loadClass('FLEA_Db_TableDataGateway');
     * </code>
     *
     * 在查找类定义文件时，类名称中的“_”会被替换为目录分隔符，
     * 从而确定类名称和类定义文件的映射关系（例如：FLEA_Db_TableDataGateway 的定义文件为
     * FLEA/Db/TableDataGateway.php）。
     *
     * loadClass() 会首先尝试从开发者指定的搜索路径中查找类的定义文件。
     * 搜索路径可以用 FLEA_Container::import() 添加，或者通过 $dirs 参数提供。
     *
     ...
     * example:
     * <code>
     * $service = FLEA_Container::getSingleton('Service_Products');
     * </code>
     *
     ...
     * <code>
     * // 注册一个对象
     * FLEA_Container::register(new MyObject(), 'my_obejct');
     * .....
     * // 稍后取出对象
     * $obj = FLEA::registry('my_object');
     * </code>
     *
     * FLEA_Container 提供一个对象注册表，开发者可以将一个对象以特定名称注册到其中。
     *
     * 当没有提供 $name 参数时，则以对象的类名称作为注册名。
     *
     * 当 $persistent 参数为 true 时，对象将被放入持久存储区。在下一次执行脚本时，
     * 可以通过 FLEA_Container::registry() 取出放入持久存储区的对象，无需重新构造对象。
     ...
     * example:
     * <code>
     * if (!FLEA_Container::isRegister('ApplicationObject')) {
     * 		FLEA_Container::loadClass('Application');
     * 		FLEA_Container::register(new Application(), 'ApplicationObject', true);
     * }
     * $app = FLEA_Container::registry('ApplicationObject');
     * </code>
     *
    ...
    static function register($obj, $name = null, $persistent = false)
    {
        ...
        require_once 'FLEA/Exception/DuplicateEntry.php';
        throw new Exception_DuplicateEntry('FLEA_Container::register($name)', $name);
    }

    /**
     * 取得指定名字的对象实例，如果指定名字的对象不存在则抛出异常
     *
     * 使用示例参考 FLEA_Container::register()。
     *
    ...
    static function registry($name)
    {
        ...
        require_once 'FLEA/Exception/NonExistentEntry.php';
        throw new Exception_NonExistentEntry('FLEA_Container::registry($name)', $name);
    }
     ...
     * example:
     * <code>
     * FLEA_Container::loadFile('Table/Products.php');
     * </code>
     *
     ...
     * FLEA_Container::import('/www/app');
     * FLEA::loadClass('Service_Products');
     * </code>
     *
     * @param string $dir
     */
