...
class FLEA_Exception extends Exception
{
    ...
    static public function t($msg)
    {
        ...
        $language = preg_replace('/[^a-z0-9\-_]+/i', '', FLEA::getAppInf('defaultLanguage'));
        if (!isset($message[$language])) {
            $message[$language] = include "/_Errors/{$language}/ErrorMessage.php";
        }

        if (isset($message[$language][$msg])) {
            return sprintf($message[$language][$msg], array_shift(func_get_args()));
        } else {
            return sptintf($msg, array_shift(func_get_args()));
        }
    }
