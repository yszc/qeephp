...
/**
 * @file
 * 定义 Helper_ImgCode 类和 Helper_ImgCodeSimple 类
 *
 * @ingroup helper
 *
 * @{
 ...
 */
class Helper_ImgCode extends QController_Helper_Abstract
{
     ...
     *     {
     *         // 在控制器中用下列代码返回一个图像验证码
     *         return $this->imgcode->make('simple', 6, 900);
     *     }
     *
     *     .....
     * }
     ...
     * <img src="<?php echo $context->url('default/imgcode'); ?>" border="0" />
     *
     * </code>
     *
     * @param int $length
     ...
     *   具体验证码样式的选项
	 */
	function make($length = 4, $lefttime = 900, $style = 'simple', array $options = null)
    {
        ...
        $this->_writeImgcodeToSession($code, $lefttime);
     ...
     * @param string $code
     *   要比对的验证码
     * @param boolean $clean_session
     *   是否在比对通过后清理 session 中保存的验证码
     *
     * @return boolean
     *   比对结果
     */
    function check($code, $clean_session = false)
    {
        $code_in_session = $this->_readImgcodeFromSession();

        if (strlen($code_in_session) == 0 || strlen($code) == 0)
        {
            return false;
        }
        if (strtolower($code_in_session) != strtolower($code))
        {
            return false;
        }
     ...
     *   比对结果
     */
    function checkCaseSensitive($code, $clean_session = false)
    {
        $code_in_session = $this->_readImgcodeFromSession();
        if (strlen($code_in_session) == 0 || strlen($code) == 0)
        {
        ...
        if ((string)$code_in_session != (string)$code)
        {
            return false;
        }
	 ...
	 * @return array
	 */
	static function hex2rgb($hex, $default = 'ffffff')
	{
        $hex = trim($hex, '#&Hh');
        $len = strlen($hex);
        if ($len == 3)
     ...
     *   验证码的有效期
     */
    protected function _writeImgcodeToSession($code, $lefttime)
    {
        if (isset($_SESSION))
        {
            $_SESSION[$this->_config['imgcode_session_key']] = $code;
            $_SESSION[$this->_config['expired_session_key']] = CURRENT_TIMESTAMP + intval($lefttime);
        }
    }
     ...
     */
    protected function _readImgcodeFromSession()
    {
        if (!isset($_SESSION))
        {
            return false;
        ...
        $key = $this->_config['imgcode_session_key'];
        $imgcode = isset($_SESSION[$key]) ? $_SESSION[$key] : '';
        $key = $this->_config['expired_session_key'];
     ...
     */
    protected function _cleanImgcodeFromSession()
    {
        if (isset($_SESSION))
        {
            $key = $this->_config['imgcode_session_key'];
...
class Helper_ImgCodeTTF
{
    ...
    (
        'alpha_thin.ttf', 'aircut.ttf', 'allcaps.ttf',
    );
