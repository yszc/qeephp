...
class QCache_Memcached {
    ...
    public function __construct(array $policy = null) {
        if (!extension_loaded('memcache')) { throw new QCache_Exception('The memcache extension must be loaded before use!'); }

        if(is_array($policy)) $this->_default_policy = array_merge($this->_default_policy, $policy);

        if (empty($this->_default_policy['servers'])) { $this->_default_policy['servers'][] = $this->_default_server; }
        ...
        foreach ($this->_default_policy['servers'] as $server) {
            ...
            if (!$result) { throw new QCache_Exception(sprintf('Connect memcached server [%s:%s] failed!', $server['host'], $server['port'])); }
        }
    }
